...
export default function AccommodationReview() {
    ...
    useEffect(() => {
        ...
        setTotalNight(Math.ceil((new Date(cookie?.roomSearch?.checkout).getTime() - new Date(cookie?.roomSearch?.checkin).getTime()) / (1000 * 3600 * 24)))
        ...
        <>
            <div className="container mx-auto" style={{
                ...
                <div className="mt-12 mb-8">
                    <h1 className="text-2xl font-bold mb-3">Kiểm tra lại đặt chỗ và thanh toán</h1>
                    <h5 className="text-md font-bold text-gray-500">Quý khách vui lòng xem lại chi tiết đặt phòng của bạn trước khi thanh toán</h5>
                </div>
                <div className="grid pb-4" style={{
                    ...
                    <div>
                        ...
                        <Card className='mt-6 hidden' title={<p className='font-bold text-xl'>Chính sách khách sạn & phòng</p>}>
                            <Collapse
                        ...
                        <Card className='mt-6 shadow' title={<p className='font-bold text-xl'>Chính Sách Lưu Trú</p>}>
                        ...
                        <div className="w-full flex justify-end mt-6">
                            <button onClick={accommodationBook} className="relative inline-flex items-center justify-center p-0.5 mb-2 me-2 overflow-hidden text-sm font-medium text-gray-900 rounded-lg group bg-gradient-to-br from-pink-500 to-orange-400 group-hover:from-pink-500 group-hover:to-orange-400 hover:text-white ">
                                <span className="relative px-5 py-2.5 transition-all ease-in duration-75 bg-white rounded-md group-hover:bg-opacity-0">
                                    Quay lại
                                </span>
                            </button>
                            <button type="button" onClick={processBooking} className="text-white bg-gradient-to-br from-green-400 to-blue-600 hover:bg-gradient-to-bl focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2" disabled={!cookie?.paymentMethod}>Tiến hành thanh toán</button>
                        </div>
                    </div>
                    <div>
                        <Card className='mb-4 shadow' title={<p className='font-bold text-xl'>Chi tiết người liên lạc</p>}>
                        ...
                        <ul className="w-full">
                            <li className='mb-2'>
                                <input type="radio" id="method_one" onClick={selectPaymentMethod} name="payment_method" value="vnpay" className="hidden peer" required defaultChecked={cookie?.paymentMethod === 'vnpay'} />
                            ...
                            <li className='mb-2'>
                                <input type="radio" id="method_two" onClick={selectPaymentMethod} name="payment_method" value="momo" className="hidden peer" defaultChecked={cookie?.paymentMethod === 'momo'} />
                            ...
                            <li className='mb-2'>
                                <input type="radio" id="method_three" onClick={selectPaymentMethod} name="payment_method" value="paypal" className="hidden peer" defaultChecked={cookie?.paymentMethod === 'paypal'}/>
